<#
    ...
    Deliberately not the dev deploy path (plugins/MoonlightPeaksMods/FormLock), which only
    exists to keep hand-built DLLs clear of Vortex during development.

    There is no test project to run: the entire mod is one Harmony patch and its config, every
    code path reads Unity and game types, so a console runner could not exercise anything
    meaningful. Verification is in TESTING.md instead.
#>
...
# The mod lives at mods/FormLock in the notes repo. Detect a standalone-repo layout too, so one
# script works in both and the two copies never diverge.
$parent   = Split-Path -Parent $modRoot
$repoRoot = if ((Split-Path -Leaf $parent) -eq 'mods') { Split-Path -Parent $parent } else { $modRoot }

$project = Join-Path $modRoot 'src\FormLock.csproj'
...
# The version is reported to players twice; a mismatch means an archive that lies about what is
# inside it.
...
Write-Host "Packing Form Lock $version"
...
$dll = Join-Path $modRoot 'src\bin\Release\netstandard2.1\FormLock.dll'
if (-not (Test-Path $dll)) { throw "Built DLL not found at $dll" }

$staging = Join-Path $env:TEMP "FormLock-pack-$([guid]::NewGuid().ToString('N'))"
$target  = Join-Path $staging 'BepInEx\plugins\FormLock'
...
$archive = Join-Path $dist "FormLock-$version.zip"
